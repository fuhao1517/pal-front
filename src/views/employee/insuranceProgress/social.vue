      ...
      <div class="search-row">
        <div class="display-flex">
          ...
          <div class="flex1 text-right pr-5">
            <el-button size="small" type="primary" class="w-60" @click="getTableData">查询</el-button>
        ...
        <div class="display-flex mt-20">
          <span class="label w-100 text-right mr-5">社保常用地：</span>
          <div class="flex1" ref="usedWidth">
            <span class="tag" :class="{active:index===userActive}" v-for="(item,index) in usedAddr" :key="item.id" @click="selectUsed('id',index)">{{item.name}}</span>
          </div>
        </div>
      </div>

      <div class="result-row" v-show="result.isShow">
        <p>社保<span>{{result.resYear}}</span>年<span>{{result.resMonth}}</span>月费用周期在保<span class="text-blue">{{result.paidTotal}}</span>人次
        </p>
      </div>
      <div>
        <dTable @fetch="getTableData" ref="dTable" style="margin-top: 0;" :tableHeight="tableHeight" :showIndex='true' :showSelection='false'>
          ...
          <el-table-column prop="account" label="社保号" width="200" :show-overflow-tooltip="true"/>
          <el-table-column prop="hkType" label="参保户口性质" width="150" :show-overflow-tooltip="true"/>
          ...
          <el-table-column prop="empTbBase" label="基数" min-width="150" :show-overflow-tooltip="true"/>
          <el-table-column prop="empPaymentAmountTotal" :formatter="formatterAmount" label="个人实付金额" min-width="150" :show-overflow-tooltip="true"/>
          <el-table-column prop="compPaymentAmountTotal" :formatter="formatterAmount" label="单位实付合计" min-width="150" :show-overflow-tooltip="true"/>
          <el-table-column prop="paymentTotal" label="合计" :formatter="formatterAmount" width="150" :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <el-popover placement="right" width="auto" trigger="hover">
                <dTable :data="scope.row.itemVO" :cancelMinHeight="true" :showSelection="false" :showIndex="false" :paging="false">
                  <el-table-column width="150" property="itemTypeName" label="险种"></el-table-column>
                  <el-table-column width="100" property="compPaymentAmount" label="单位金额" :formatter="formatterAmount"></el-table-column>
                  <el-table-column width="100" property="empPaymentAmount" label="个人金额" :formatter="formatterAmount"></el-table-column>
                  <el-table-column width="100" property="compAndEmpPaymentAmountTatol" label="合计" :formatter="formatterAmount"></el-table-column>
                </dTable>
                <el-button type="text" slot="reference" v-text="scope.row.paymentTotal"></el-button>
              </el-popover>
            </template>
          </el-table-column>
        </dTable>
    ...
    methods: {
      ...
      getTableData(type) {
        ...
        this.$refs.dTable.fetch({
          query: params,
          method: 'post',
          url: '/api/employee/getEmployeeSocialRoster'
        })
      ...
      getAddr() {
        ...
        this.$http({
          ...
          this.$nextTick(function () {
            that.usedAddr = caclUsedAddr(that, data, 'usedWidth')
          })
        })
      },
      /* 导出 */
      handleExport() {
        ...
        that.$downloadFile('/api/employee/exportSocialRoster', 'post', {
