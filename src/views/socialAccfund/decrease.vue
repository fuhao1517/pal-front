  ...
  <div class="spl-container content">
    ...
    <div v-show="selectEmployee.idCard!=''">
              ...
              <el-row class="row-bg m-form mt-20" justify="space-around" :span="24">
                <el-form :inline="true" :model="socialForm" ref="socialForm" label-width="120px">
                  ...
                  <el-col :span="rowSpan">
                    <el-form-item label="费用年月:" prop="fyDate"
                      ...
                      <el-date-picker v-model="socialForm.fyDate" value-format="yyyy-MM" class="date-editor2" type="month" placeholder="选择日期" @change="getSocailProducts()" clearable>
                      </el-date-picker>
                    </el-form-item>
                  </el-col>
                  <el-col :span="rowSpan">
                  ...
                  <el-col :span="rowSpan">
                    <el-form-item prop="comment" :rules="[{ validator: (rule, value, callback)=>checkSocialForm(rule, value, callback, '请选择离职原因'), trigger:'change'}]">
                      <template slot="label">
                        <span class="required-pre">离职原因:</span>
                      </template>
                      <el-select v-model="socialForm.comment" class="w-p100" clearable>
                        <el-option v-for="(it, ind) in socialForm.exitReasonList" :key="ind" :label="it"
                                   :value="it"></el-option>
                      </el-select>
                    </el-form-item>
                  </el-col>
                ...
                <el-col :span="rowSpan" v-for="(item,index) in socialForm.fieldList" :key="index">
                  ...
                  <div :key="refreshSocialKey">
                    <el-form :inline="true" :model="socialForm.fieldList[index]" :ref="'socialFieldForm'+index" label-width="120px">
                    <el-form-item :label="item.declareColumnName + ':'" prop="value" :rules="[{ validator: (rule, value, callback)=>checkSocialForm(rule, value, callback, '请输入'+item.declareColumnName), trigger:'blur'},{ validator: (rule, value, callback)=>checkSocialForm(rule, value, callback, '请输入'+item.declareColumnName), trigger:'change'}]">
                      <template slot="label">
              ...
              <el-row class="row-bg m-form mt-20" justify="space-around" :span="24">
                <el-form :inline="true" :model="accfundForm" ref="accfundForm" label-width="120px">
                  ...
                  <el-col :span="rowSpan">
                    <el-form-item label="费用年月:" prop="fyDate"
                      ...
                      <el-date-picker v-model="accfundForm.fyDate" value-format="yyyy-MM" class="date-editor2" type="month" placeholder="选择日期"  @change="getAccfundProducts()" clearable>
                      </el-date-picker>
                    </el-form-item>
                  </el-col>
                  <el-col :span="rowSpan">
                  ...
                  <el-col :span="rowSpan">
                    <el-form-item prop="comment" :rules="[{ validator: (rule, value, callback)=>checkAccfundForm(rule, value, callback, '请选择离职原因'), trigger:'change'}]">
                      <template slot="label">
                        <span class="required-pre">离职原因:</span>
                      </template>
                      <el-select v-model="accfundForm.comment" class="w-p100" clearable>
                        <el-option v-for="(it, ind) in accfundForm.exitReasonList" :key="ind" :label="it"
                                   :value="it"></el-option>
                      </el-select>
                    </el-form-item>
                  </el-col>
                ...
                <el-col :span="rowSpan" v-for="(item,index) in accfundForm.fieldList" :key="index">
                  ...
                  <div :key="refreshAccfundKey">
                    <el-form :inline="true" :model="accfundForm.fieldList[index]" :ref="'accfundFieldForm'+index" label-width="120px">
                    <el-form-item :label="item.declareColumnName + ':'" prop="value" :rules="[{ validator: (rule, value, callback)=>checkAccfundForm(rule, value, callback, '请输入'+item.declareColumnName), trigger:'blur'},{ validator: (rule, value, callback)=>checkAccfundForm(rule, value, callback, '请输入'+item.declareColumnName), trigger:'change'}]">
                      <template slot="label">
                        <span class="required-pre">{{item.declareColumnName}}:</span>
                      </template>

                      <!--非日期-->
                      <el-input v-model="accfundForm.fieldList[index].value" v-if="item.inputType!='40009006' && item.scopes.length<1" clearable  @blur="fieldInputBlur(index, 'accfundForm')"></el-input>
      ...
      <div style="padding: 20px;border: 1px solid #DDDDDD; border-top: none">
        <el-Table :data="socialTable" ref="socialTable">
          ...
          <el-table-column label="单位缴纳" align="center">
            <el-table-column prop="compBaseMin" label="单位基数" min-width="120" header-align="left" align="left"
                             :show-overflow-tooltip="true"></el-table-column>
          ...
          <el-table-column label="个人缴纳" align="center">
            <el-table-column prop="empBaseMin" label="个人基数" min-width="120" header-align="left" align="left"
                             :show-overflow-tooltip="true"></el-table-column>
        ...
        <!-- 公积金 -->
        <el-Table v-show="accfundTable.length>0" :data="accfundTable" ref="accfundTable" :show-header="false">
          <template slot="empty"><span></span></template>
          ...
          <el-table-column prop="compBaseMin" min-width="120" :show-overflow-tooltip="true"></el-table-column>
          ...
          <el-table-column prop="empBaseMin" min-width="120" :show-overflow-tooltip="true"></el-table-column>
...
export default {
  ...
  data () {
    return {
      ...
      refreshSocialKey: new Date().getTime(),
      refreshAccfundKey: new Date().getTime(),
      socialForm: {
        addrArr: [],
        socialAddrId: '',
        socialAddrName: '',
  ...
  created () {
    ...
    this.getFormKey()
    this.getAddrArr()
    this.getExitReasonList(1)
    this.getExitReasonList(2)
  },
  ...
  methods: {
    ...
    // 离职原因
    getExitReasonList (type) {
      // type:社保1公积金2
      let addrId = ''
      let isHave = false
      if (type == 1) {
        addrId = this.socialForm.socialAddrId
      } else if (type == 2) {
        addrId = this.accfundForm.accfundAddrId
      }
      if (this.$lodash.isEmpty(addrId)) {
        return false
      }
      this.$http({
        url: '/api/socialAccfund/' + type + '/resignationReason?addrId=' + addrId,
        method: 'get'
      }).then(({ data }) => {
        if (type == 1) {
          this.socialForm.exitReasonList = data.data[0]
          for (let i = 0; i < data.data[0].length; i++) {
            if (data.data[0][i] == this.socialForm.comment) {
              isHave = true
              break
            }
          }
          if (!isHave) {
            this.socialForm.comment = ''
          }
        } else if (type == 2) {
          this.accfundForm.exitReasonList = data.data[0]
          for (let i = 0; i < data.data[0].length; i++) {
            if (data.data[0][i] == this.accfundForm.comment) {
              isHave = true
              break
            }
          }
          if (!isHave) {
            this.accfundForm.comment = ''
          }
        }
      })
    },
    ...
    getSocailProducts () {
      // type:社保1公积金2
      // changeType 申报类型（1：增员，2：减员，3：调基，4：方案变更）
        ...
        if (!isHave) {
          if (data.data[0].length == 1) {
            ...
            if (this.accfundForm.accfundProductId != '') {
              ...
              this.accfundTable = []
              this.getSummaries('accfundTable')
            }
          }
        }
      }).catch((data) => {
    ...
    handleValid () {
      ...
      for (let j = 0; j < this.accfundForm.fieldList.length; j++) {
        this.$refs['accfundFieldForm' + j][0].validate((valid) => {
    ...
    getSummaries (tableName) {
      let that = this
      let data = []
      if (tableName === 'socialTable') {
        data = this.socialTable
      } else if (tableName === 'accfundTable') {
        data = this.accfundTable
      }
      ...
      data.forEach((column, index) => {
        compBaseMinItem = calFee(column.compBaseMin, column.compRatioMin, column.compCalAccuCode)
        empBaseMinItem = calFee(column.empBaseMin, column.empRatioMin, column.empCalAccuCode)
        column.dCompPay = compBaseMinItem
        column.dEmpPay = empBaseMinItem
    ...
    checkSocialRequired () {
      ...
      if (this.$lodash.isEmpty(socialAddrId) && this.$lodash.isEmpty(productCode) && this.$lodash.isEmpty(fyDate)) {
