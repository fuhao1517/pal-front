...
http.interceptors.response.use(response => {
  switch (response.data.code) {
    case 5000:
      initMessageBox(response.data.msg ? response.data.msg : '会话已经过时，请重新登录')
      clearLoginInfo()
      router.push('/login')
      break
  }
  if (response.data.code && response.data.code !== '10') {
    let errorMsg = response.data.msg ? response.data.msg : response.data.data[0].msg ? response.data.data[0].msg : response.data.data[0] ? response.data.data : global.errorMsg
    if (errorMsg == '系统异常' || errorMsg == '系统发生了异常' || errorMsg.indexOf('Exception') > -1 || errorMsg.indexOf('exception') > -1) {
      showErrorDialog()
    } else {
